...
template<typename nodeType, typename edgeType>
void initResidualGraph(vector<nodeType*> *Graph, vector<nodeType*> *residualGraph, bool isVertexCapacity = false)
{
    if(false == isVertexCapacity)
    {
        ...
        for(size_t i = 0; i < noOfNodes;i++)
        {
            ...
            for(size_t j = 0; j < tempNoOfEdges ;j++)
            {
                ...
                {
                    tempEdge = new edgeType((*residualGraph)[i],(*residualGraph)[tempEdges[j]->second->Id - 1],1,tempEdges[j]->residualCapacity);
    ...
    {
        size_t noOfNodes = Graph->size();
        for(size_t i = 0; i < 2 * noOfNodes;i++)
            (*residualGraph)[i] = new nodeType (i + 1);
        for(size_t i = 0; i < noOfNodes;i++)
        {
            vector<edgeType*> tempEdges = (*Graph)[i]->edges;
            ...
            for(size_t j = 0; j < tempNoOfEdges ;j++)
            {
                if(tempEdges[j]->residualCapacity > 0)
                {
                    if(i != 0)
                    {
                        tempEdge = new edgeType((*residualGraph)[i + noOfNodes],(*residualGraph)[tempEdges[j]->second->Id - 1],1,tempEdges[j]->residualCapacity);
                        (*residualGraph)[i + noOfNodes]->edges.push_back(tempEdge);
                    }
                    else
                    {
                        tempEdge = new edgeType((*residualGraph)[i],(*residualGraph)[tempEdges[j]->second->Id - 1],1,tempEdges[j]->residualCapacity);
            ...
            if(0 != i)
            {
                tempEdge = new edgeType((*residualGraph)[i],(*residualGraph)[i + noOfNodes],1, 1 /*Unit residual capacity for vertex*/);
                (*residualGraph)[i]->edges.push_back(tempEdge);
            }
        }
    }
...
void decompilePath(vector<edgeType*> *Path,int noOfNodes = 0,bool isVertexCapacity = false)
{
    if(false == isVertexCapacity)
    {
