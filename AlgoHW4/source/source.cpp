...
int main()
{
	...
	int shortestShortestPath = 10000000;
	char line[1000],*temp;
	temp = line;
	inpLine(temp);
	...
	if(hasNegativeCycle == false)
	{
		...
		for(int i=0;i<noOfNodes;i++)
		{
			...
			for(int j=0;j<noOfNodes;j++)
			{
				...
				if(shortestShortestPath > Cache[i][j])
					shortestShortestPath = Cache[i][j];
			}
		}
	}
	...
	//OUTPUT
	if(hasNegativeCycle == false)
		cout<<shortestShortestPath<<endl;
	else
		cout<<"NULL"<<endl;
	return 0;
}
